...
{
    // REMOTE_AUDIO_STATE_STOPPED is not used at this version. Ignore this value.
    // REMOTE_AUDIO_STATE_STOPPED = 0,  // Default state, audio is started or remote user disabled/muted audio stream
    REMOTE_AUDIO_STATE_RUNNING = 1,  // Running state, remote audio can be played normally
    REMOTE_AUDIO_STATE_FROZEN = 2,    // Remote audio is frozen, probably due to network issue.
};
    
enum QUALITY_LIMITATION_REASON
{
    LIMITATION_REASON_NONE = 0, // The bitrate and/or frame rate is not limited.
    LIMITATION_REASON_BANDWIDTH = 1, // The bitrate and/or frame rate is primarily limited due to congestion cues during bandwidth estimation.
};
  
enum RENDER_VIDEO_FRAME_TYPE
{
    RENDER_VIDEO_FRAME_TYPE_DEFAULT = 0,
    RENDER_VIDEO_FRAME_TYPE_IOSCVPIXELBUFFER = 1,
};
...
typedef struct LiveTranscoding {
    ...
    LiveTranscoding()
        ...
        , videoCodecType(VIDEO_CODEC_DEFAULT)
        , userCount(0)
...
public:
    ...
    */
    ...
    /**
	* reported local audio stats
	* @param [in] stats
    *        the latest local audio stats
    */
	...
	*/
    ...
    /**
    * reported remote audio stats
    * @param [in] stats
	*        the latest remote audio stats
	*/
...
//    virtual void onStreamError(int streamId, int code, int parameter, const char* message, size_t length) {}
    ...
    */
    ...
    virtual void onLocalVideoQualityLimited(const LocalVideoStats& stats, QUALITY_LIMITATION_REASON reason) {
        (void)stats;
        (void)reason;
    }
...
- <0: Failure.
    ...
    */
    ...
    int setRenderVideoFrameParameters(RENDER_VIDEO_FRAME_TYPE renderFrameType) {
      return setParameters("{\"che.video.render_video_frame_type\":%d}", renderFrameType);
    }
#endif
